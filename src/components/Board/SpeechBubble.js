...
export default function SpeechBubble(context, playerX, playerY) {
	...
	this.overflow = SpeechBubble.OVERFLOW_ELLIPSIS;
...
SpeechBubble.prototype.formatText = function() {
    ...
    for (let j = 0; j < words.length; j++)
        if (words[j].length > 15) {
            words[j] = words[j].substring(0, 14) + "...";
        }

	this.context.font = this.fontSize + "px " + this.font;

	const lines = [words[0]];
	let lineLength = this.context.measureText(words[0]).width;
...
SpeechBubble.prototype.getSafeSpace = function() {
	const left = this.panelBounds.left + this.padding + this.cornerRadius;
	const top = this.panelBounds.top + this.padding + this.cornerRadius;
	const width = this.panelBounds.width - this.padding * 2 - this.cornerRadius * 2;
	const height = this.panelBounds.height - this.padding * 2 - this.cornerRadius * 2;
...
SpeechBubble.prototype.drawText = function(lines) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		else if (this.textAlign === SpeechBubble.ALIGN_CENTER)
...
SpeechBubble.prototype.drawPanelWall = function(panelSide, tailLocation, toX, toY) {
	if (panelSide === tailLocation.side && !this.panelBounds.inBounds(this.target.x, this.target.y)) {
		const tailBaseVector = panelSide.drawVector.scale(this.tailBaseWidth);
		const start = tailLocation.sub(tailBaseVector);
		const end = tailLocation.add(tailBaseVector);
...
SpeechBubble.prototype.drawTail = function(panelSide, start, end) {
	...
	if (this.tailStyle === SpeechBubble.TAIL_STRAIGHT) {
		...
		const deltaVec = this.target.sub(this.getTailLocation());
		deltaVec.x = deltaVec.x * panelSide.normalVector.x;
		deltaVec.y = deltaVec.y * panelSide.normalVector.y;
		const tailHalfLength = Math.pow(deltaVec.mag(), 0.7);
...
SpeechBubble.prototype.getTailLocation = function() {
	...
	const boundsCenter = this.panelBounds.getCenter();
	const relativeTargetX = this.target.x - boundsCenter.x;
	const relativeTargetY = this.target.y - boundsCenter.y;

	const targetAspectRatio = relativeTargetX / relativeTargetY;
	const boundsAspectRatio = this.panelBounds.width / this.panelBounds.height;
	...
	const location = new SpeechBubble.Vector(x, y);
	location.side = side;
...
SpeechBubble.BezierCurve.prototype.draw = function() {
	for (let i = 0; i <= this.resolution; i++) {
		const point = this.getPoint(i/this.resolution);
		this.context.lineTo(point.x, point.y);
	};
};

SpeechBubble.BezierCurve.prototype.getPoint = function(t) {
	let point = new SpeechBubble.Vector();
	let n = this.points.length - 1;

	for (let i = 0; i < this.points.length; i++) {
		const scale = Math.pow(1-t, n-i) * Math.pow(t, i) * SpeechBubble.Utils.binomialCoefficient(n, i);
		const scaledVector = this.points[i].scale(scale);
